...
for (i in 1:length(tau_day)) {
  ...
  plot(SPD[[i]]$xGrid, SPD[[i]]$result, xlim = xlim_rnd[i, ], ylim = ylim_rnd[i, 
      ], xlab = "Spot price", ylab = "Density", type = "l", main = paste("Risk neutral density,", 
...
s3d_rnd  = scatterplot3d(df_rnd, type = "h", color = col_rnd, main = "Risk Neutral Densities for different maturities", 
...
abline(v = axis.Date(1, at = seq(as.Date("2011-12-22"), as.Date("2014-12-22"), 
                                 "years"), format = "%Y/%m/%d"), col = "lightgray", lty = "dotted", 
       lwd = par("lwd"))
